# 源接口健康检测报告

最近更新时间：2026-08-24 09:01 CST

**总源数:** 60 | **检测关键词:** 逐玉

| 状态 | 资源名称 | 地址 | API | 搜索功能 | 成功次数 | 失败次数 | 成功率 | 最近7天趋势 |
|------|---------|-----|-----|---------|---------:|--------:|-------:|--------------|
| 🚨 | 雨哥哥资源 | [Link](http://cj.baozi66.top:66) | [Link](http://cj.baozi66.top:66/api.php/provide/vod) | ❌ | 18 | 12 | 60.0% | ❌❌❌❌❌❌❌ |
| ❌ | 豆瓣资源 | [Link](https://dbzy.tv) | [Link](https://dbzy.tv/api.php/provide/vod) | ❌ | 28 | 2 | 93.3% | ✅✅✅✅✅❌❌ |
| ❌ | 豆瓣资源 | [Link](https://dbzy.tv) | [Link](https://caiji.dbzy5.com/api.php/provide/vod) | ❌ | 24 | 6 | 80.0% | ❌✅✅✅✅❌❌ |
| ✅ | 樱花资源 | - | [Link](https://m3u8.apiyhzy.com/api.php/provide/vod) | ❌ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 无尽资源 | [Link](https://wujinzy.net) | [Link](https://api.wujinapi.me/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 快车资源阿 | [Link](https://kuaichezy.com) | [Link](https://caiji.kuaichezy.org/api.php/provide) | ❌ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | TV-麒麟资源 | [Link](https://www.qilinzyz.com/) | [Link](https://www.qilinzyz.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 建安资源站 | - | [Link](http://154.219.117.232:9981/jacloudapi.php/provide/vod/) | 不匹配 | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬360 资源 | [Link](https://360zy.com) | [Link](https://360zyzz.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 天涯资源 | - | [Link](https://tyyszy.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 天涯影视资源 | [Link](https://tyyszyapi.com) | [Link](https://tyyszyapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 无水印资源网采集接口 | - | [Link](https://api.wsyzy.net/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 索尼资源 | [Link](https://suonizy.net) | [Link](https://suoniapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬魔都动漫 | [Link](https://caiji.moduapi.cc) | [Link](https://caiji.moduapi.cc/api.php/provide/vod) | ❌ | 28 | 2 | 93.3% | ✅✅✅✅❌❌✅ |
| ✅ | 非凡影视 | [Link](http://ffzy5.tv) | [Link](http://ffzy5.tv/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬魔都资源 | [Link](https://www.moduzy.net) | [Link](https://www.mdzyapi.com/api.php/provide/vod) | ✅ | 28 | 2 | 93.3% | ✅✅✅✅❌❌✅ |
| ✅ | 🎬-爱奇艺- | [Link](https://iqiyizyapi.com) | [Link](https://iqiyizyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬iKun资源 | [Link](https://ikunzy.com) | [Link](https://ikunzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬猫眼资源 | [Link](https://www.maoyanzy.com) | [Link](https://api.maoyanapi.top/api.php/provide/vod) | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬暴风资源 | [Link](https://bfzy.tv) | [Link](https://bfzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬最大资源 | [Link](https://zuida.xyz) | [Link](https://api.zuidapi.com/api.php/provide/vod) | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬新浪资源 | [Link](https://xinlangapi.com) | [Link](https://api.xinlangapi.com/xinlangapi.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬速播资源 | [Link](https://www.subozy.com) | [Link](https://subocaiji.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬金鹰点播 | [Link](https://jinyingzy.com) | [Link](https://jyzyapi.com/provide/vod/from/jinyingyun/at/json) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬U酷影视 | [Link](https://www.ukuzy.com) | [Link](https://api.ukuapi88.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬红牛资源 | [Link](https://www.hongniuzy.com) | [Link](https://www.hongniuzy2.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬如意资源 | [Link](https://www.ryzyw.com) | [Link](https://cj.rycjapi.com/api.php/provide/vod) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 🎬豪华资源 | [Link](https://www.haohuazy.com) | [Link](https://hhzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬百度云zy | [Link](https://bdzy1.com) | [Link](https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 360资源 | - | [Link](https://360zy.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 1080资源 | [Link](https://api.1080zyku.com) | [Link](https://api.1080zyku.com/inc/api_mac10.php) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | U酷资源 | [Link](https://api.ukuapi.com) | [Link](https://api.ukuapi.com/api.php/provide/vod) | ✅ | 29 | 1 | 96.7% | ❌✅✅✅✅✅✅ |
| ✅ | 无尽资源 | [Link](https://api.wujinapi.cc) | [Link](https://api.wujinapi.cc/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 丫丫点播 | [Link](https://cj.yayazy.net) | [Link](https://cj.yayazy.net/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 无尽资源 | [Link](https://api.wujinapi.com) | [Link](https://api.wujinapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 无尽资源 | [Link](https://api.wujinapi.net) | [Link](https://api.wujinapi.net/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 最大点播 | [Link](http://zuidazy.me) | [Link](http://zuidazy.me/api.php/provide/vod) | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 牛牛点播 | [Link](https://api.niuniuzy.me) | [Link](https://api.niuniuzy.me/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 虎牙资源 | [Link](https://www.huyaapi.com) | [Link](https://www.huyaapi.com/api.php/provide/vod) | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 金鹰资源 | [Link](https://jyzyapi.com) | [Link](https://jyzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 闪电资源 | [Link](https://sdzyapi.com) | [Link](https://sdzyapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 红牛资源 | [Link](https://www.hongniuzy3.com) | [Link](https://www.hongniuzy3.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 闪电资源 | [Link](https://xsd.sdzyapi.com) | [Link](https://xsd.sdzyapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 优质资源 | [Link](https://api.yzzy-api.com) | [Link](https://api.yzzy-api.com/inc/apijson.php) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | TV-神马云 | [Link](https://api.1080zyku.com) | [Link](https://api.1080zyku.com/inc/apijson.php/) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 辣椒资源黄黄 | - | [Link](https://apilj.com/api.php/provide) | ❌ | 27 | 3 | 90.0% | ❌✅✅✅✅✅✅ |
| ✅ | 优质资源库1080zyk6.com高清 | - | [Link](https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod) | ❌ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 快车资源阿 | - | [Link](https://caiji.kuaichezy.org/api.php/provide/vod) | ❌ | 25 | 5 | 83.3% | ❌✅✅✅✅✅✅ |
| ✅ | 🔅飞速资源ᴴ | [Link](http://fszy1.com) | [Link](http://fszy1.com/api.php/provide/vod) | ❌ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 🔅樱花资源ᴴ | [Link](https://yhzy.cc) | [Link](https://yhzy.cc/api.php/provide/vod) | ✅ | 25 | 5 | 83.3% | ❌✅✅✅✅✅✅ |
| ✅ | U酷资源 | - | [Link](https://api.ukuapi88.com/api.php/provide/art/?ac=list) | 不匹配 | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 🎬量子资源 | [Link](https://cj.lzcaiji.com) | [Link](https://cj.lzcaiji.com/api.php/provide/vod) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 量子资源 | - | [Link](https://cj.lziapi.com/api.php/provide/vod) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 🎬 无广1线 (非凡) | [Link](https://yonghu.ffzyapi8.com) | [Link](https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/) | ✅ | 25 | 5 | 83.3% | ❌✅✅✅✅✅✅ |
| ✅ | 🎬 ikunzy资源 | [Link](https://www.ikunzy.com) | [Link](https://www.ikunzy.com/api.php/provide/vod/) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 猫眼资源 | [Link](https://www.maoyanzy.com) | [Link](https://api.maoyanapi.top/api.php/provide) | ❌ | 25 | 5 | 83.3% | ❌✅✅✅✅✅✅ |
| ✅ | 杏吧资源 | [Link](https://sex8zy8.com) | [Link](https://xingba222.com/api.php/provide) | ❌ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 神马资源 | [Link](https://img.smdyw.top) | [Link](https://img.smdyw.top/api.php/provide/vod) | ❌ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 新浪 | [Link](https://api.xinlangapi.com) | [Link](https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |
| ✅ | 1080资源 | [Link](https://1080zyk.com) | [Link](https://api.yzzy-api.com/inc/api_mac10.php) | ✅ | 26 | 4 | 86.7% | ❌✅✅✅✅✅✅ |

<details>
<summary>📜 点击展开查看历史检测数据 (JSON)</summary>

```json
[
  {
    "date": "2026-07-25",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-26",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-27",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-28",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-29",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-30",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-07-31",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-01",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-02",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-03",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-04",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-05",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-06",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-07",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-08",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-09",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-10",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-11",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-12",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-13",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-14",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-08-15",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-08-16",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-08-17",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": false,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-08-19",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-20",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-21",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-22",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-23",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-24",
    "keyword": "逐玉",
    "results": [
      {
        "name": "樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "TV-麒麟资源",
        "api": "https://www.qilinzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "建安资源站",
        "api": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "天涯影视资源",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无水印资源网采集接口",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "非凡影视",
        "api": "http://ffzy5.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jyzyapi.com/provide/vod/from/jinyingyun/at/json",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://pz.v88.qzz.io/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "360资源",
        "api": "https://360zy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "豆瓣资源",
        "api": "https://dbzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "1080资源",
        "api": "https://api.1080zyku.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "丫丫点播",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "无尽资源",
        "api": "https://api.wujinapi.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "最大点播",
        "api": "http://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "牛牛点播",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "金鹰资源",
        "api": "https://jyzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "红牛资源",
        "api": "https://www.hongniuzy3.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "TV-神马云",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "辣椒资源黄黄",
        "api": "https://apilj.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "优质资源库1080zyk6.com高清",
        "api": "https://api.yzzy-api.com/inc/ldg_api_all.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "快车资源阿",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔅飞速资源ᴴ",
        "api": "http://fszy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔅樱花资源ᴴ",
        "api": "https://yhzy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "U酷资源",
        "api": "https://api.ukuapi88.com/api.php/provide/art/?ac=list",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "量子资源",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 无广1线 (非凡)",
        "api": "https://yonghu.ffzyapi8.com/api.php/provide/vod/from/ffm3u8/at/json/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬 ikunzy资源",
        "api": "https://www.ikunzy.com/api.php/provide/vod/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "杏吧资源",
        "api": "https://xingba222.com/api.php/provide",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "雨哥哥资源",
        "api": "http://cj.baozi66.top:66/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "神马资源",
        "api": "https://img.smdyw.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/josn",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "1080资源",
        "api": "https://api.yzzy-api.com/inc/api_mac10.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      }
    ]
  }
]
```
</details>
